...
plot_track_duration = function(trackdata=trackdata, cex.lab=1, cex.axis=1){
  ...
  Date_D = as.numeric(difftime(Time, min(Time, na.rm=T), units='days'))
  trackdata = cbind(trackdata, Date_D)
  trackdata = trackdata[!is.na(trackdata$Date_D), ] # 2018-01-01 00:00:00 return NA
  ...
  axis(side=1, at=trackdata$Date_D[ at ], labels = as.Date(trackdata$Date_D[ at ], origin = min(Time, na.rm=T)), cex.axis=cex.axis)
...
#' @examples
#'
#' ind = trackdata[trackdata$ID=="hooded06_683_BFU076" & trackdata$Year == 2018,]
#' plot_direction(ind)
#'
